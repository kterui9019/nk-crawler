...
export const procedure = async (horseId: string, raceId: string) => {
	...
	races.forEach((_, idx) => {
		const raceLink = dom.querySelector(selectors.beforeResult.raceLink(idx+1))

		if (raceLink?.attributes.href.substr(6, 12) === raceId) {
			res.before1 = dom.querySelector(selectors.beforeResult.raceLink(idx+2))?.attributes.href.substr(6, 12) || ""
			res.before2 = dom.querySelector(selectors.beforeResult.raceLink(idx+3))?.attributes.href.substr(6, 12) || ""
		}
	})
